...
{
    public class _PropertyAmenityComponentPartial(IHttpClientFactory _clientFactory):ViewComponent
    {
        public async Task<IViewComponentResult> InvokeAsync()
        {
            var client = _clientFactory.CreateClient("MyApiClient");
            var responseMessage = await client.GetAsync("PropertyAmenities/ResultPropertyAmenityByStatusTrue?id=1");
